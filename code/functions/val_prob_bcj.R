...
# p = val.prob(
#   p = GAM[[tar]]$prd$test,
#   y = data$validation[[tar]],
#   pl = F
# ) 
# 
# st_ggdat <- enframe(p$stats)
# 
# np_ggdat <- p$smooth %>% 
#   select(ideal_x, model_x, y) %>% 
#   gather(variable, x, -y) %>% 
#   mutate(
#     variable = recode(
#       variable, 
#       ideal_x = "Ideal",
#       model_x = "Non-parametric"
#     )
#   )
# 
# lg_ggdat <- p$smooth %>% 
#   select(y=probl_x, x=logit_x) %>% 
#   mutate(variable = 'Logistic')
# 
# bind_rows(lg_ggdat, np_ggdat) %>%
#   ggplot(aes(x=x, y=y, col = variable)) + 
#   geom_line() +
